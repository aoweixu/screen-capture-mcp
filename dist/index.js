...
function getScreenLayout() {
    const result = execSync(`powershell -NoProfile -NonInteractive -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Screen]::AllScreens | ForEach-Object { Write-Output ('' + $_.Bounds.X + ',' + $_.Bounds.Y + ',' + $_.Bounds.Width + ',' + $_.Bounds.Height) }"`, { encoding: "utf-8", timeout: 10000 });
    return result.trim().split(/\r?\n/).map((line) => {
        const [x, y, width, height] = line.split(",").map(Number);
        return { x, y, width, height };
    });
}
function captureSingleScreen(screen) {
